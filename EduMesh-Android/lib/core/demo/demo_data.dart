...
class DemoData {
  ...
  static dynamic _mockFor(String path, Map<String, dynamic>? queryParams) {
    ...
    if (p == '/register') {
      return {'id': 'LUMINA_01-DEMO${DateTime.now().millisecondsSinceEpoch.toString().substring(8)}'};
    }
    ...
    if (p == '/student/change-password') {
      return {'status': 'success'};
    }
    
    // --- Catalog / Resources ---
    if (p == '/api/catalog' || p == '/resources') {
      return [
    ...
    if (p == '/teacher/subjects' || p == '/teacher/subjects/delete') {
      return {'status': 'success'};
    }
    if (p.startsWith('/teacher/profiles')) {
      return {'status': 'success'};
    }
    if (p.startsWith('/teacher/change-password')) {
      return {'status': 'success'};
    }
    if (p.startsWith('/teacher/disable-default-admin')) {
      return {'status': 'success'};
    }
    if (p.startsWith('/teacher/upload')) {
      return {'status': 'success'};
    }
    if (p.startsWith('/teacher/upload-zim')) {
      return {'status': 'success', 'imported': ['demo_article_1.html']};
    }
    if (p.startsWith('/teacher/import-server-file')) {
      return {'status': 'success'};
    }
    if (p.startsWith('/teacher/resources/')) {
      return {'status': 'success'};
    }
    
    // Student reset/delete — dynamic paths like /teacher/scholars/reset-password/{id}
    if (p.contains('/scholars/reset-password') || p.contains('/scholars/delete')) {
      return {'status': 'success'};
    }
    if (p.startsWith('/teacher/reset-password/')) {
      return {'status': 'success'};
    }
    
    // --- Stats & System ---
    if (p == '/stats') {
      return {'storage_percent': 45.2, 'battery_percent': 82};
    }
    if (p == '/system/stats') {
      return {'status': 'healthy'};
    }
    if (p == '/system/sync-time') {
      return {'status': 'ok'};
    }
    ...
    if (p.startsWith('/api/admin/logs/download')) {
    ...
    if (p == '/zim/page') {
      return {'id': queryParams?['id'] ?? 'DMO001', 'html': '<html><body><h1>Demo ZIM Article</h1><p>This is demo content.</p></body></html>'};
    }
    
    // --- Limits ---
    if (p == '/api/limits') {
      return {'zim_upload_max_size': 5000000000};
    }
    ...
    // --- Logout, etc. ---
    if (p == '/logout' || p == '/generate_204') {
      return {'status': 'ok'};
    }
    
    // --- Student activity ---
    if (p == '/student/activity') {
      return List.generate(25, (i) {
        final actions = ['view', 'search', 'download', 'watch'];
        final subjects = ['Mathematics', 'Physics', 'Chemistry', 'Biology', 'English', 'History', 'Geography', 'Computer Science'];
        return {
          'id': i,
          'action': actions[i % actions.length],
          'resource_id': 'res_$i',
          'metadata': subjects[i % subjects.length],
          'timestamp': '2026-05-30T${(10 - i % 10).toString().padLeft(2, '0')}:${(i * 3 % 60).toString().padLeft(2, '0')}:00',
        };
      });
    }
    
    if (p == '/student/study/start') {
      return {'status': 'ok', 'session_id': 1};
    }
    
    if (p == '/student/study/end') return {'status': 'ok'};

    if (p == '/student/analytics') {
      return {
        'study_minutes_today': 87,
        'study_minutes_this_week': 1240,
        'study_minutes_this_month': 4200,
        'streak_days': 12,
        'resources_saved': 6,
        'subjects': [
          {'name': 'Mathematics', 'minutes': 320},
          {'name': 'Physics', 'minutes': 260},
          {'name': 'Chemistry', 'minutes': 195},
          {'name': 'Biology', 'minutes': 140},
          {'name': 'English', 'minutes': 175},
          {'name': 'History', 'minutes': 150},
        ],
      };
    }

    if (p == '/student/profile/update') {
      return {'status': 'ok'};
    }

    if (p == '/student/profile/icon') {
      return {'status': 'ok', 'filename': 'LUMINA_01-DEMOSCHOLAR_icon.png'};
    }
