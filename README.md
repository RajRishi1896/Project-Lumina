...
**Current maturity.** This is a proof of concept that shipped. The system is feature-complete for single-classroom and school-lab deployment. It passes an 89-test pytest suite (11 files) and a 116-test Flutter suite (12 files), `dart analyze` at 0 errors and 0 warnings, and has run end-to-end on emulators and development hardware. The 250-concurrent-student concurrency target is a design figure validated only at small scale; a repeatable benchmark harness is not in scope. What exists works. What doesn't exist was never needed for the PoC.
...
- **Mutation queue.** `MutationQueue` persists profile updates and activity events when the hub is unreachable and replays them in order on reconnect, dropping only after ten retries (quiz submissions must survive long outages), immediate drops on permanent 4xx rejections. All student-state mutations route through this queue; project convention forbids direct fire-and-forget calls.
...
- **Offline quiz grading.** Server sends answer keys with quiz data for offline grading. Quiz attempts persist locally and sync to server on reconnect. The timer pauses when the app backgrounds and resumes when the student returns.
...
- **Token renewal.** Three-tier session fallback (session token, 7-day refresh token, 365-day persistent key); the Dio interceptor walks the tiers on 401/403 before logging out. Expiry for unused refresh/persistent tokens is enforced at cleanup; an expired-but-not-yet-pruned token can still exchange once, a known PoC gap.
...
- `PopScope` double-back-to-exit with a 2-second window (first press shows an auto-expiring toast; second press exits).
...
- **Soft-delete lifecycle.** Deleting a resource sets `status = 'deleted'` plus `deleted_at` (30-day recycle bin); an hourly purge hard-deletes file and row. Deleted resources vanish from the catalog, search, and the direct-ID detail endpoint. (ZIM archive deletion is the exception: any teacher or admin can permanently delete an archive and its files, with no recycle bin.)
- **Rate limiting.** Per-IP sliding window at 200 requests/minute (configurable), exempting localhost, static assets, `/ping`, and ZIM GETs; ZIM uploads have a separate 5-per-5-minute cap and logins are capped at 10 failed attempts per minute per IP.
- **Content standards.** Uploads validate against a controlled subject taxonomy and grades 1 to 13 (0 reserved for pre-primary, 13 for bridging and exam prep; "General" also accepted); source and license metadata are collected at upload with defaults (`Unknown` / `Internal Only`) when omitted. Duplicate uploads on `title + subject + grade + language + resource_type` are rejected with a `409` response plus an explicit `force_upload` override. Upload language metadata accepts `en`, `hi`, `kn`, `fr` (Tamil/Telugu UI exists but is not yet an upload language option). Quiz creation requires every question to carry a correct answer (`correct_answer` or `correct_answers`); answerless quizzes are rejected with `400` because the app cannot load or grade them.
...
- **No external dependencies.** The server starts and serves every endpoint with no internet connection.
...
- **Auth.** Opaque session tokens (`LUMINA_HUB-`, `LUMINA_REF-`, `LUMINA_PER-` prefixes) validated against SQLite session rows with the three-tier renewal described above; bcrypt-hashed passwords server-side; the offline-only login path uses a separate local SHA-256 credential that never shares the server hash.
...
Client: the Flutter app mirrors catalog and progress state in its own local SQLite (schema version 18, per-profile file, fresh-install schema with no migration chain); see `EduMesh-Android/lib/core/storage/db_helper.dart`.
...
**Test coverage vs. offline reliability.** Development time went to offline reliability (mutation queue, download atomicity, Keystore recovery) rather than test volume. The server suite is 89 tests across 11 files; the Flutter suite is 116 tests across 12 files (scheduler, flip card, animation system, profile flows, profile switcher, course model, course locks, bookmark sync, download groups, stepper, mini-player, smoke): enough to make refactoring safe, not exhaustive. This was the correct trade-off for a PoC.
...
| Local database | SQLite via `sqflite` | Offline cache, pending queues (schema v18) |
...
| Auth (server) | bcrypt + opaque session tokens (SQLite-backed) | Password hashing, role-based access |
...
Direct dependencies of the Flutter app are listed in `pubspec.yaml`; server dependencies in `requirements.txt`. No CDN-served assets and no external APIs appear in the offline-critical path.
...
sudo systemctl start lumina-hotspot  # Activates the WiFi hotspot
```

`setup_hub.sh` provisions system dependencies, a Python venv, UFW firewall rules, dnsmasq captive-portal DNS via NetworkManager, systemd services, lid-close sleep disable, nightly reboot, and a 65,535 file-descriptor limit. It needs internet for the initial `apt`/`pip` pass and is idempotent for offline re-runs when package caches are warm; `ffmpeg` and the Python ZIM bindings must be present for thumbnails and ZIM processing.
...
cd "Debian Server" && pytest         # 89 tests
```
